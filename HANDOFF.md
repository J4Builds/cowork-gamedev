...
2. **proto-004-tetris shipped (macro + juice).** Macro: 10×20 board, 7 tetrominoes, 7-bag randomizer, simple rotation with 1/2-cell wall kicks (no SRS, no T-spins), ghost piece, next-piece preview, hard drop, soft drop, 0.5s lock delay with rotation reset, line clear + cascade, 1/3/5/8 hundred scoring × level, NES-curve gravity ramp, top-out, restart, title/pause, localStorage BEST. Juice (no visual animations per playtest — only the asks): cell size 30→40, ENTER-restarts-from-pause, muted palette (Tailwind 600/700 instead of 400-level neons), all-synth SFX (lock tap as mid-range triangle wood-block, line-clear scaling 1→4 with the 4-line being an ascending arpeggio, level-up sting, descending game-over), and a light-techno music loop (kick on quarter notes + hi-hat on off-beats, constant 120 BPM, fades cleanly on pause). Live at https://j4builds.github.io/cowork-gamedev/projects/proto-004-tetris/.

3. **Audio iteration lessons (Tetris juice pass).** Audio went through five passes before landing: (a) initial volume too quiet — bumped master 0.5→0.8 and SFX peaks ~2×; (b) lock thump at 120→60 Hz sine was *inaudible* on the user's speakers (below physical driver range) — moved to mid-range 380→180 Hz triangle "wood-block" character, fixed instantly; (c) ambient sine pad with intensity-driven chord shifts was "terrible" — too foreground despite being intentionally textural; (d) light techno (kick + hi-hat + bass, three intensity tiers) got closer; (e) user landed on consistent tier-2 (kick + hi-hat only, no scaling, constant tempo) and called it done. Two takeaways worth saving as principles.
